/*
  CC1101.cpp - Advanced CC1101 module library (V3.0.0)
*/
#include "CC1101.h"
#include <math.h>
#include <Arduino.h>
...
bool CC1101::WaitMiso(uint16_t timeout_ms) { 
...
void CC1101::SpiWriteBurstReg(byte addr, byte *buffer, byte num) {
    SpiStart();
    if (WaitMiso()) {
        _spi->transfer(addr | WRITE_BURST);
        for (byte i = 0; i < num; i++) _spi->transfer(buffer[i]);
    }
    SpiEnd();
}
...
void CC1101::SpiReadBurstReg(byte addr, byte *buffer, byte num) {
    ...
    if (WaitMiso()) {
        _spi->transfer(addr | READ_BURST);
        for (byte i = 0; i < num; i++) buffer[i] = _spi->transfer(0);
    }
    SpiEnd();
}
...
void CC1101::Init(SPIClass* spiBus) {
    _spi = spiBus;
    Init();
}
